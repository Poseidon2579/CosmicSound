...
export default function ProfileSettingsForm({ user }: { user: User }) {
    ...
    return (
        ...
        ? allGenres.filter(g => g.toLowerCase().includes(genreSearch.toLowerCase()))
                : allGenres.slice(0, 20); // Show only top 20 or first few if no search
    ...
    const toggleGenre = (genre: string) => {
        const current = preferences?.genres || [];
                ...
                // Also remove associated combinatory prefs
                ...
                setSelectedGenreForDecades(genre); // Open popover to select decades
        }
    };
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Tus Géneros (+100)</label>
                                ...
                                {filteredGenres.map(genre => {
                                    ...
                                    return (
                                        <div key={genre} className="relative group">
                                            ...
                                            >
                                                {genre}
                                                {comboDecades.length > 0 && (
                                                    <span className="bg-white/20 px-1.5 rounded-md text-[9px]">{comboDecades.join(', ')}</span>
                                                )}
                                                {selected && (
                                                    <span
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Filtro General de Décadas</label>
                            <div className="flex flex-wrap gap-2">
                                {allDecades.map(decade => {
