...
import { Rocket, Mail, Lock, User as UserIcon, LogIn, Sparkles } from "lucide-react";
            ...
            <div className="relative z-10 flex flex-col min-h-screen">
                <header className="flex items-center justify-between px-8 py-8 max-w-7xl mx-auto w-full">
                    ...
                    <div className="flex items-center gap-8">
                        <button onClick={() => setIsLogin(!isLogin)} className="text-xs font-bold tracking-widest uppercase opacity-50 hover:opacity-100 transition-opacity">
                    ...
                    <section className="w-full max-w-md mx-auto space-y-10 py-12">
                        <div className="text-center space-y-3">
                            <h2 className="text-4xl md:text-5xl font-bold tracking-tight">
                                {isLogin ? 'Welcome back to the Void' : 'Join the Cosmic Choir'}
                            </h2>
                            <p className="text-white/40 text-sm font-medium">
                                {isLogin ? 'Re-calibrate your auditory sensors.' : 'Establish your presence in the universe.'}
                            </p>
                        </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                >
                                    {isLoading ? (
                                        ...
                                        <>
                                            <span>{isLogin ? 'Enter the Void' : 'Begin Journey'}</span>
                                            {isLogin ? <LogIn className="size-4" /> : <Sparkles className="size-4" />}
                                        </>
                                    )}
                                </button>
                            </form>
                        ...
                        <div className="text-center pt-4">
                            <p className="text-sm text-white/40">
                                {isLogin ? 'First journey?' : 'Already recorded?'}
                                <button
                                ...
                                >
                                    {isLogin ? 'Signal presence' : 'Enter Void'}
                                </button>
                            </p>
                        </div>
                    </section>
